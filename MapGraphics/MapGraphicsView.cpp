...
MapGraphicsView::MapGraphicsView(MapGraphicsScene *scene, MapObjectsManager *manager, QWidget *parent) :
    QWidget(parent), _manager(manager)
{
    ...
    connect(manager,
            &MapObjectsManager::managerModeChanged,
            this,
            &MapGraphicsView::setDragModeByManager);
...
void MapGraphicsView::setScene(MapGraphicsScene * scene)
{
    ...
    */
    ...
    connect(this, SIGNAL(UpdateAllObjects()), childScene, SLOT(handleZoomLevelChanged()));
...
{
    if (_manager->_tempObject != nullptr) {
        switch (_manager->state()) {
        case MapObjectsManager::GeoObjectCreation: {
            QPointF mousePoint = mapToScene(_childView->mapFromGlobal(QCursor::pos()));
            auto temp = static_cast<MapGeoObject *>(_manager->_tempObject);
            temp->setPos(mousePoint);
            if (temp) { temp->setGeoObject(); }
            break;
        }
        case MapObjectsManager::MarkCreation: {
            QPointF mousePoint = mapToScene(_childView->mapFromGlobal(QCursor::pos()));
            auto temp = static_cast<MarkObject *>(_manager->_tempObject);
            temp->setPos(mousePoint);
            if (temp) { temp->setMark(); }
            break;
        }
        case MapObjectsManager::CircleObjectCreation: {
            QPointF mousePoint = mapToScene(_childView->mapFromGlobal(QCursor::pos()));
            auto temp = static_cast<CircleObject *>(_manager->_tempObject);
            temp->setPos(mousePoint);
            if (temp) { temp->redrawRequested(); }
            break;
        }
        }
    }
...
void MapGraphicsView::handleChildMouseRelease(QMouseEvent *event)
{
    ...
    case MapObjectsManager::GeoObjectCreation: {
        ...
        temp->setPos(mousePoint);
        temp->setGeoObject();
        _manager->addObjectToScene();
        emit SendGeoObject(temp);
        _manager->_tempObject = nullptr;
//        _manager->createObject();
    ...
    case MapObjectsManager::CircleObjectCreation: {
        ...
        auto temp = static_cast<CircleObject *>(_manager->_tempObject);
        temp->setPos(mousePoint);
        temp->redrawRequested();
        _manager->addObjectToScene();
        emit SendAdvancObject(temp);
        _manager->_tempObject = nullptr;
        break;
    }
    case MapObjectsManager::MarkCreation: {
    ...
    case MapObjectsManager::RouteCreation: {
        QPointF mousePoint = mapToScene(_childView->mapFromGlobal(QCursor::pos()));
        auto temp = static_cast<RouteObject *>(_manager->_tempObject);
        if (!temp->checkPosBegin()){
            temp->setPosBegin(mousePoint);
        } else {
            temp->setPosEnd(mousePoint);
            temp->setZoomLevel(_zoomLevel);
            connect(temp,
                    &RouteObject::getMovementObject,
                    _manager,
                    &MapObjectsManager::linkRouteAndObject);
            _manager->addObjectToScene();
            temp->requestRoute();
            _manager->_tempObject = nullptr;
        }
...
void MapGraphicsView::handleChildViewScrollWheel(QWheelEvent *event)
{
    ...
    if( _manager->state() == MapObjectsManager::NoCreation) {
        this->setDragMode(MapGraphicsView::ScrollHandDrag);
    }
    if (event->delta() > 0)
...
QPointer<MapObjectsManager> MapGraphicsView::manager() const
{
    return _manager;
}

void MapGraphicsView::setManager(const QPointer<MapObjectsManager> &manager)
{
    _manager = manager;
}

void MapGraphicsView::UpdateMapGraphicsObjects()
{
    _childView->invalidateScene();
    emit UpdateAllObjects();
}

void MapGraphicsView::setDragModeByManager(const MapObjectsManager::ManagerMode mode)
{
    switch (mode) {
    case MapObjectsManager::MarkCreation:{ setDragMode(NoDrag); break; }
    case MapObjectsManager::RouteCreation:{ setDragMode(NoDrag); break; }
    case MapObjectsManager::MarkRemove:{ setDragMode(ScrollHandDrag); break; }
    case MapObjectsManager::LinkRoute:{ setDragMode(ScrollHandDrag); break; }
    case MapObjectsManager::CustomRouteCreation:{ setDragMode(NoDrag); break; }
    default:
