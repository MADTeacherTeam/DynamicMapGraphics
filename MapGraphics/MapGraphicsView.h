...
public:
    explicit MapGraphicsView(MapGraphicsScene * scene=0, MapObjectsManager * manager=0, QWidget * parent = 0);
    ...
    QPointer<MapObjectsManager> manager() const;
    void setManager(const QPointer<MapObjectsManager> &manager);

    void UpdateMapGraphicsObjects();

signals:
    void zoomLevelChanged(quint8 nZoom);
    void SendGeoObject(MapGeoObject*);
    void SendAdvancObject(MapGraphicsObject *);
    void UpdateAllObjects();
    
public slots:
    void setDragModeByManager(MapObjectsManager::ManagerMode const mode);
...
private:
    ...
    QPointer<MapObjectsManager> _manager;
